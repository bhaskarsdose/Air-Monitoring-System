...
#define USE_AVG
// Arduino pin numbers.
const int sharpLEDPin = D4;   // Nodemcu digital pin 1 connect to sensor LED.
const int sharpVoPin = A0;   // Nodemcu analog pin 1 connect to sensor Vo.
...
float Vo = VoRaw;
  ...
  static char PM[7];
    dtostrf(dustDensity, 6, 2, PM);
    client.publish("/esp8266/PM", PM);
...
//last of dust sensor
  ...
  if (now - lastMeasure > 1000) {
   ...
   // float g = bme.readGas();
    ...
    client.publish("/esp8266/AQIscore", AQIscore);   //publishes the AQI score or value to the server

    //static char AQItext[7];
    //dtostrf(AQI_text1, 6, 2, AQItext);
    client.publish("/esp8266/AQItext", IAQtext);
    Serial.println(IAQtext);

    // Publishes Temperature and Humidity values
    client.publish("/esp8266/temperature", temperature);
    
    client.publish("/esp8266/pressure", pressure);
...
//  Serial.print(score);
  ...
  Serial.println("------------------------------------------------");
  delay(2000);
    Serial.println("i am here bhaskar");
